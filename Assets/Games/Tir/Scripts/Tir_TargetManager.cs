...
namespace Tir
{
    public class Tir_TargetManager : MonoBehaviour
    {
        ...
        private void SpawnTarget(RectTransform transform, TirTeam team)
        {
            ...
            _lastMultiplicator *= 0.99f - (PlayerPrefs.GetInt(Tir_SceneObject.DifficultyKey) * 0.03f);
            targetBehaviour.TimerMultiplicator = _lastMultiplicator;
            targetBehaviour.transform.localScale = Vector3.one * (1f - (PlayerPrefs.GetInt(Tir_SceneObject.DifficultyKey) / 4f));
